...
-- Primeiro, habilitar RLS na tabela se nu00e3o estiver habilitado
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'RLS habilitado para a tabela asaas_webhook_events';
  ELSE
    RAISE NOTICE 'RLS ju00e1 estu00e1 habilitado para a tabela asaas_webhook_events';
  END IF;
END
$$;

-- Remover a polu00edtica existente se necessu00e1rio e criar uma nova
DO $$
BEGIN
  -- Verificar se a polu00edtica admin_policy existe
  IF EXISTS (
    ...
    DROP POLICY admin_policy ON public.asaas_webhook_events;
    RAISE NOTICE 'Polu00edtica admin_policy removida';
  END IF;
  
  -- Criar a nova polu00edtica
  CREATE POLICY admin_policy ON public.asaas_webhook_events
    USING (auth.uid() IN (
      SELECT user_id FROM public.user_types WHERE type = 'admin'
    ));
  RAISE NOTICE 'Polu00edtica admin_policy criada';
  
  -- Verificar se a polu00edtica service_role_policy existe
  IF EXISTS (
    SELECT FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'asaas_webhook_events' 
    AND policyname = 'service_role_policy'
  ) THEN
    DROP POLICY service_role_policy ON public.asaas_webhook_events;
    RAISE NOTICE 'Polu00edtica service_role_policy removida';
  END IF;
  
  -- Criar a polu00edtica service_role_policy
  CREATE POLICY service_role_policy ON public.asaas_webhook_events
    USING (current_setting('role') = 'service_role');
  RAISE NOTICE 'Polu00edtica service_role_policy criada';
END
$$;
