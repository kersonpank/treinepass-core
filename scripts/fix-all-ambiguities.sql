-- Script para corrigir todas as ambiguidades na funu00e7u00e3o process_asaas_webhook
...
-- 2. Criar a funu00e7u00e3o atualizada com a correu00e7u00e3o de todas as ambiguidades
...
DECLARE
    ...
    debug_data JSONB; -- Renomeado de debug_info para debug_data
...
BEGIN
    ...
    -- Inicializar debug_data se a coluna existir
    IF EXISTS (
    ...
    IF event_type LIKE 'PAYMENT_%' AND payment_data IS NOT NULL THEN
        ...
        -- Atualizar debug_data se existir
        IF debug_data IS NOT NULL THEN
    ...
    IF EXISTS (
        ...
        SET 
            processed = TRUE,
            processed_at = NOW(),
            debug_info = debug_data -- AQUI ESTu00c1 A CORREu00c7u00c3O: usando debug_data em vez de debug_info
        ...
        IF EXISTS (
            ...
            IF EXISTS (
                ...
                SET 
                    ...
                    debug_info = COALESCE(debug_data, '{}'::JSONB) || jsonb_build_object('fatal_error', error_msg) -- AQUI ESTu00c1 A CORREu00c7u00c3O: usando debug_data em vez de debug_info
                WHERE id = webhook_event_id;
            ELSE
                UPDATE public.asaas_webhook_events
                SET 
...
DO $$
BEGIN
    RAISE NOTICE 'A funu00e7u00e3o process_asaas_webhook foi atualizada com a correu00e7u00e3o de todas as ambiguidades';
END;
$$;
